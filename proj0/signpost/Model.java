...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        ArrayList<Integer> checkAll = new ArrayList<Integer>();
        _solnNumToPlace = new Place[last + 1];
        _board = new Sq[_width][_height];
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                if (checkAll.contains(solution[i][j]) || solution[i][j] > last
                        || solution[i][j] < 1) {
                    throw badArgs("IllegalArgumentException");
                }
                checkAll.add(solution[i][j]);
                _solnNumToPlace[solution[i][j]] = pl(i, j);
                if (solution[i][j] == 1 || solution[i][j] == last) {
                    _board[i][j] =
                            new Sq(i, j, solution[i][j], true,
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                ...
                for (Place temp : _board[i][j]._successors) {
                    if (_board[temp.x][temp.y]._predecessors == null) {
                        _board[temp.x][temp.y]._predecessors = new PlaceList();
                        _board[temp.x][temp.y]._predecessors.add(pl(i, j));
                    } else {
                        _board[temp.x][temp.y]._predecessors.add(pl(i, j));
                    }
                }
            ...
            for (int j = 0; j < _height; j++) {
                if (model._board[i][j]._successor != null) {
                    _board[i][j]._successor =
                            this.get(model._board[i][j]._successor.pl);
                }
                if (model._board[i][j]._predecessor != null) {
                    _board[i][j]._predecessor =
                            this.get(model._board[i][j]._predecessor.pl);
                }
                if (model._board[i][j]._head != null) {
                    _board[i][j]._head = this.get(model._board[i][j]._head.pl);
                }
            }
        }
    }
    ...
    void init(int width, int height) {
        ...
        _allSuccSquares = Place.successorCells(_width, _height);
    }
    ...
    boolean autoconnect() {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                if (_board[i][j].successor() == null
                        && _board[i][j].sequenceNum() != 0) {
                    for (Sq next: _allSquares) {
                        if (next.sequenceNum()
                                == _board[i][j].sequenceNum() + 1) {
                            _board[i][j].connect(next);
                            change = true;
                        }
                    }
                }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                Sq next = _successor;
                int seq = _sequenceNum + 1;
                while (next != null) {
                    next._sequenceNum = seq;
            ...
            if (s1._sequenceNum != 0) {
                Sq prev = this;
                int seq = s1._sequenceNum - 1;
                while (prev != null && seq > 0) {
                    prev._sequenceNum = seq;
         ...
         * return true iff a Square and its successors
         * have at least one fixed number*/
        boolean checkSuccsFixed() {
            Sq temp = this;
            while (temp != null) {
                if (temp.hasFixedNum()) {
                    return true;
        ...
        /** Helper function for disconnect().
         * return true iff a Square and its predecessors
         * have at least one fixed number*/
        boolean checkPredsFixed() {
            Sq temp = this;
            while (temp != null) {
                if (temp.hasFixedNum()) {
                    return true;
        ...
        /** Helper function for disconnect().
         * set its successors' head to this head*/
        void setSuccsHeads() {
            Sq temp = this;
            while (temp.successor() != null) {
                temp._successor._head = this.head();
                temp = temp._successor;
            }
        }
    ...
    /** _allSuccSquares[x][y][dir] is a sequence of all queen moves possible
     *  on the board of in direction dir from (x, y).*/
