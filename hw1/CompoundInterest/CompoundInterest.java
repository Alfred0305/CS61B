...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        double value = presentValue * Math.pow((1+rate*0.01),numYears(targetYear));
        return value;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return value * Math.pow(1-inflationRate*0.01, numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        for(int i = 0; i <= numYears(targetYear); i++){
            multipliers += Math.pow(1+rate*0.01,numYears(targetYear)-i);
        }
        return perYear * multipliers;
    }
