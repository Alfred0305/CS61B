...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        double[] ele = new double[m[0].length];
        result[0] = m[0];
        for(int i = 1; i < m.length;i++){
            for(int j = 0; j < m[0].length; j++){
                if(j>0 && j<m[0].length-1){
                    ele[j] = Math.min(result[i-1][j-1],Math.min(result[i-1][j],result[i-1][j+1])) + m[i][j];
                }else if(j==0){
                    ele[j] = Math.min(result[i-1][j], result[i-1][j+1]) + m[i][j];
                }else{
                    ele[j] = Math.min(result[i-1][j-1], result[i-1][j]) + m[i][j];
                }
                result[i] = ele;
            }
            ele = new double[m[0].length];
        }
        return result;
    }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if(orientation == Orientation.VERTICAL){
            ...
            return helper(accumulateVertical(helper(m)));
        }
    }

    public static double[][] helper(double[][] m){
        double[][] result = new double[m[0].length][];
        double[] ele = new double[m.length];
        for(int i = 0; i < m[0].length; i++){
            for(int j = 0; j < m.length;j++){
                ele[j] = m[j][i];
            }
            result[i] = ele;
            ele = new double[m.length];
        }
        return result;
    }
